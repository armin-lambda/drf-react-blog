...
const PostList = () => {
    ...
    return (
        <div className={styles.container}>
            ...
            {!loading && !error && posts.length === 0 && <div className="state-box">Nothing to show...</div>}

            {!loading && !error && posts.map((post) => (
                <div className={styles.card} key={post.id}>
                    <a href={`/${post.slug}`} className={styles.title}>{post.title}</a>
                    <span className={styles.date}>Published at: {formatDateTime(post.created_at)}</span>
